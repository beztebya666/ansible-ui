...
(async()=>{
  ...
  const shoot=async(lang,re,file)=>{
    ...
    await p.evaluate(rs=>{const els=[...document.querySelectorAll("h2")];const h=els.find(e=>new RegExp(rs).test(e.textContent||""));if(h)h.scrollIntoView({block:"center"});},re);
    await s(700);
    await p.screenshot({path:`/out/${file}`}); console.log("shot",file);
  };
  await shoot("ru","Приватный Galaxy","galaxy-ru.png");
  await shoot("en","Private Galaxy","galaxy-en.png");
  await b.close();
