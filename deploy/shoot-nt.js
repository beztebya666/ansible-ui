...
(async()=>{
  ...
  // open "add notification channel"
  const ok=await p.evaluate(()=>{const b=[...document.querySelectorAll("button")].find(x=>/Добавить|канал|Новый канал|уведомлен/i.test(x.textContent)&&x.querySelector("svg"));if(b){b.click();return b.textContent;}return null;});
  await s(900);
  await p.screenshot({path:"/out/nt-form.png"}); console.log("shot ok="+ok);
  await b.close();
