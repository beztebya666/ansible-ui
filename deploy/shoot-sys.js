...
(async()=>{
  ...
  const p=await b.newPage(); await p.setViewport({width:1300,height:1200}); const s=ms=>new Promise(r=>setTimeout(r,ms));
  ...
  await p.goto(`${base}/settings`,{waitUntil:"networkidle2"}); await s(1500);
  // scroll to the System information card
  await p.evaluate(()=>{const els=[...document.querySelectorAll("h2")];const h=els.find(e=>/Информация о системе/.test(e.textContent||""));if(h)h.scrollIntoView({block:"start"});});
  await s(900);
  await p.screenshot({path:"/out/system.png"}); console.log("shot");
  await b.close();
