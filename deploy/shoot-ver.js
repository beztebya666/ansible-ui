...
(async()=>{
  ...
  const p=await b.newPage(); await p.setViewport({width:1300,height:820,deviceScaleFactor:2}); const s=ms=>new Promise(r=>setTimeout(r,ms));
  ...
  const sb=await p.$('input'); if(sb){await sb.click();await sb.type("-app");}
  await s(900);
  await p.screenshot({path:"/out/ver-list.png",clip:{x:250,y:120,width:1040,height:420}}); console.log("list");
  await b.close();
