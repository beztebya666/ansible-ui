...
(async()=>{
  ...
  const cap=async(lang)=>{
    ...
    await p.evaluate(()=>{const t=[...document.querySelectorAll("button,a,[role='tab']")].find(e=>/Инвентари|Inventories/i.test(e.textContent||""));if(t)t.click();});
    await s(1000);
    const r=await p.evaluate(()=>{
      const lbl=[...document.querySelectorAll("*")].find(e=>e.textContent.trim()==="ssh-inv"&&e.children.length===0);
    ...
    await s(500);
    await p.screenshot({path:`/out/connkey-${lang}.png`}); console.log("shot",lang,r);
